"""Tests for ArchitectureReviewer FSM implementation.

TDD RED PHASE: These tests are designed to FAIL initially because
the FSM implementation doesn't exist yet. The ArchitectureReviewer
...
class TestArchitectureFSMInitialization:
    """Test ArchitectureReviewer FSM initialization."""

    def test_reviewer_exists(self):
        """Verify ArchitectureReviewer can be imported."""
        from iron_rook.review.agents.architecture import ArchitectureReviewer

        assert ArchitectureReviewer is not None

    def test_architecture_reviewer_initializes_with_fsm(self):
        """Verify ArchitectureReviewer initializes with FSM attributes."""
        from iron_rook.review.agents.architecture import ArchitectureReviewer
    ...
    def test_architecture_reviewer_initial_phase_is_intake(self):
    ...
    def test_architecture_reviewer_get_agent_name_returns_architecture(self):
        """Verify get_agent_name returns 'architecture'."""
        from iron_rook.review.agents.architecture import ArchitectureReviewer
...
class TestArchitectureFSMTransitions:
    """Test architecture FSM state transitions."""

    def test_valid_transitions_defined(self):
        """Verify VALID_TRANSITIONS class attribute exists."""
        from iron_rook.review.agents.architecture import ArchitectureReviewer

        assert hasattr(ArchitectureReviewer, "VALID_TRANSITIONS") or hasattr(
            ArchitectureReviewer, "FSM_TRANSITIONS"
        )
    ...
    def test_intake_to_plan_valid(self):
        ...
        transitions = getattr(ArchitectureReviewer, "VALID_TRANSITIONS", None) or getattr(
            ArchitectureReviewer, "FSM_TRANSITIONS", {}
        )
...
class TestArchitecturePhaseMethods:
    """Test architecture phase methods implementation."""
...
class TestArchitecturePrefersDirectReview:
    """Test prefers_direct_review method for architecture reviewer."""
...
class TestArchitecturePhaseLoggerIntegration:
    """Test phase logger integration for thinking output."""

    @patch("iron_rook.review.agents.architecture.SimpleReviewAgentRunner")
    @pytest.mark.asyncio
    async def test_intake_phase_returns_valid_output(self, mock_runner_class):
        ...
        mock_runner.run_with_retry.return_value = """{
            "phase": "intake",
            "data": {
                "summary": "Architecture changes detected",
...
class TestArchitectureFilePatternsAndTools:
    """Test architecture reviewer file patterns and tools."""

    def test_get_relevant_file_patterns_returns_architecture_patterns(self):
        """Verify get_relevant_file_patterns returns architecture-related patterns."""
        from iron_rook.review.agents.architecture import ArchitectureReviewer

        reviewer = ArchitectureReviewer()
        patterns = reviewer.get_relevant_file_patterns()

        # Verify architecture-relevant patterns
        assert "**/*.py" in patterns

    def test_get_allowed_tools_returns_architecture_tools(self):
        """Verify get_allowed_tools returns architecture-related tools."""
        from iron_rook.review.agents.architecture import ArchitectureReviewer
...
class TestArchitectureReviewOutputGeneration:
    """Test ReviewOutput generation from architecture FSM."""
    ...
    def test_build_review_output_from_check_creates_valid_output(self):
                    ...
                    "medium": [
                        {
                            ...
                            "description": "Cross-module dependency introduced",
                            "evidence": [{"type": "file_ref", "path": "src/test.py"}],
                            "recommendations": ["Refactor to use interface"],
                        }
                    ],
                    "low": [],
                },
        ...
        assert output.agent == "architecture"
...
class TestArchitectureInvalidTransitions:
...
class TestArchitectureFullFSMExecution:
