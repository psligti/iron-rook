"""Tests for TelemetryMetricsReviewer FSM implementation.

TDD RED PHASE: These tests are designed to FAIL initially because
the FSM implementation doesn't exist yet. The TelemetryMetricsReviewer
...
class TestTelemetryFSMInitialization:
    """Test TelemetryMetricsReviewer FSM initialization."""

    def test_reviewer_exists(self):
        """Verify TelemetryMetricsReviewer can be imported."""
        from iron_rook.review.agents.telemetry import TelemetryMetricsReviewer

        assert TelemetryMetricsReviewer is not None

    def test_telemetry_reviewer_initializes_with_fsm(self):
        """Verify TelemetryMetricsReviewer initializes with FSM attributes."""
        from iron_rook.review.agents.telemetry import TelemetryMetricsReviewer
    ...
    def test_telemetry_reviewer_initial_phase_is_intake(self):
    ...
    def test_telemetry_reviewer_get_agent_name_returns_telemetry(self):
        """Verify get_agent_name returns 'telemetry'."""
        from iron_rook.review.agents.telemetry import TelemetryMetricsReviewer
...
class TestTelemetryFSMTransitions:
    """Test telemetry FSM state transitions."""

    def test_valid_transitions_defined(self):
        """Verify VALID_TRANSITIONS class attribute exists."""
        from iron_rook.review.agents.telemetry import TelemetryMetricsReviewer

        assert hasattr(TelemetryMetricsReviewer, "VALID_TRANSITIONS") or hasattr(
            TelemetryMetricsReviewer, "FSM_TRANSITIONS"
        )
    ...
    def test_intake_to_plan_valid(self):
        ...
        transitions = getattr(TelemetryMetricsReviewer, "VALID_TRANSITIONS", None) or getattr(
            TelemetryMetricsReviewer, "FSM_TRANSITIONS", {}
        )
...
class TestTelemetryPhaseMethods:
    """Test telemetry phase methods implementation."""
...
class TestTelemetryPrefersDirectReview:
    """Test prefers_direct_review method for telemetry reviewer."""
...
class TestTelemetryPhaseLoggerIntegration:
    """Test phase logger integration for thinking output."""

    @patch("iron_rook.review.agents.telemetry.SimpleReviewAgentRunner")
    @pytest.mark.asyncio
    async def test_intake_phase_returns_valid_output(self, mock_runner_class):
        ...
        mock_runner.run_with_retry.return_value = """{
            "phase": "intake",
            "data": {
                "summary": "Telemetry changes detected",
...
class TestTelemetryFilePatternsAndTools:
    """Test telemetry reviewer file patterns and tools."""

    def test_get_relevant_file_patterns_returns_telemetry_patterns(self):
        """Verify get_relevant_file_patterns returns telemetry-related patterns."""
        from iron_rook.review.agents.telemetry import TelemetryMetricsReviewer

        reviewer = TelemetryMetricsReviewer()
        patterns = reviewer.get_relevant_file_patterns()

        # Verify telemetry-relevant patterns
        assert "**/*.py" in patterns

    def test_get_allowed_tools_returns_telemetry_tools(self):
        """Verify get_allowed_tools returns telemetry-related tools."""
        from iron_rook.review.agents.telemetry import TelemetryMetricsReviewer
...
class TestTelemetryReviewOutputGeneration:
    """Test ReviewOutput generation from telemetry FSM."""
            ...
            "data": {
                "findings": {
                    ...
                    "medium": [
                        {
                            "severity": "medium",
                            "title": "Missing logging detected",
                            "description": "Critical path lacks observability",
                            "evidence": [{"type": "file_ref", "path": "src/test.py"}],
                            "recommendations": ["Add structured logging"],
                        }
                    ],
                    "low": [],
                },
...
class TestTelemetryInvalidTransitions:
...
class TestTelemetryFullFSMExecution:
