"""Tests for LintingReviewer FSM implementation.

TDD RED PHASE: These tests are designed to FAIL initially because
the FSM implementation doesn't exist yet. The LintingReviewer
...
class TestLintingFSMInitialization:
    """Test LintingReviewer FSM initialization."""

    def test_reviewer_exists(self):
        """Verify LintingReviewer can be imported."""
        from iron_rook.review.agents.linting import LintingReviewer

        assert LintingReviewer is not None

    def test_linting_reviewer_initializes_with_fsm(self):
        """Verify LintingReviewer initializes with FSM attributes."""
        from iron_rook.review.agents.linting import LintingReviewer
    ...
    def test_linting_reviewer_initial_phase_is_intake(self):
    ...
    def test_linting_reviewer_get_agent_name_returns_linting(self):
        """Verify get_agent_name returns 'linting'."""
        from iron_rook.review.agents.linting import LintingReviewer
...
class TestLintingFSMTransitions:
    """Test linting FSM state transitions."""

    def test_valid_transitions_defined(self):
        """Verify VALID_TRANSITIONS class attribute exists."""
        from iron_rook.review.agents.linting import LintingReviewer

        assert hasattr(LintingReviewer, "VALID_TRANSITIONS") or hasattr(
            LintingReviewer, "FSM_TRANSITIONS"
        )
    ...
    def test_intake_to_plan_valid(self):
        ...
        transitions = getattr(LintingReviewer, "VALID_TRANSITIONS", None) or getattr(
            LintingReviewer, "FSM_TRANSITIONS", {}
        )
...
class TestLintingPhaseMethods:
    """Test linting phase methods implementation."""
...
class TestLintingPrefersDirectReview:
    """Test prefers_direct_review method for linting reviewer."""
...
class TestLintingPhaseLoggerIntegration:
    """Test phase logger integration for thinking output."""

    @patch("iron_rook.review.agents.linting.SimpleReviewAgentRunner")
    @pytest.mark.asyncio
    async def test_intake_phase_returns_valid_output(self, mock_runner_class):
        ...
        mock_runner.run_with_retry.return_value = """{
            "phase": "intake",
            "data": {
                "summary": "Linting changes detected",
...
class TestLintingFilePatternsAndTools:
    """Test linting reviewer file patterns and tools."""

    def test_get_relevant_file_patterns_returns_linting_patterns(self):
        """Verify get_relevant_file_patterns returns linting-related patterns."""
        from iron_rook.review.agents.linting import LintingReviewer

        reviewer = LintingReviewer()
        patterns = reviewer.get_relevant_file_patterns()

        # Verify linting-relevant patterns
        assert "**/*.py" in patterns

    def test_get_allowed_tools_returns_linting_tools(self):
        """Verify get_allowed_tools returns linting-related tools."""
        from iron_rook.review.agents.linting import LintingReviewer
...
class TestLintingReviewOutputGeneration:
    """Test ReviewOutput generation from linting FSM."""
...
class TestLintingInvalidTransitions:
...
class TestLintingFullFSMExecution:
