"""Tests for PerformanceReliabilityReviewer FSM implementation.

TDD RED PHASE: These tests are designed to FAIL initially because
the FSM implementation doesn't exist yet. The PerformanceReliabilityReviewer
...
class TestPerformanceFSMInitialization:
    """Test PerformanceReliabilityReviewer FSM initialization."""

    def test_reviewer_exists(self):
        """Verify PerformanceReliabilityReviewer can be imported."""
        from iron_rook.review.agents.performance import PerformanceReliabilityReviewer

        assert PerformanceReliabilityReviewer is not None

    def test_performance_reviewer_initializes_with_fsm(self):
        """Verify PerformanceReliabilityReviewer initializes with FSM attributes."""
        from iron_rook.review.agents.performance import PerformanceReliabilityReviewer
    ...
    def test_performance_reviewer_initial_phase_is_intake(self):
    ...
    def test_performance_reviewer_get_agent_name_returns_performance(self):
        """Verify get_agent_name returns 'performance'."""
        from iron_rook.review.agents.performance import PerformanceReliabilityReviewer
...
class TestPerformanceFSMTransitions:
    """Test performance FSM state transitions."""

    def test_valid_transitions_defined(self):
        """Verify VALID_TRANSITIONS class attribute exists."""
        from iron_rook.review.agents.performance import PerformanceReliabilityReviewer

        assert hasattr(PerformanceReliabilityReviewer, "VALID_TRANSITIONS") or hasattr(
            PerformanceReliabilityReviewer, "FSM_TRANSITIONS"
        )
    ...
    def test_intake_to_plan_valid(self):
        ...
        transitions = getattr(PerformanceReliabilityReviewer, "VALID_TRANSITIONS", None) or getattr(
            PerformanceReliabilityReviewer, "FSM_TRANSITIONS", {}
        )
...
class TestPerformancePhaseMethods:
    """Test performance phase methods implementation."""
...
class TestPerformancePrefersDirectReview:
    """Test prefers_direct_review method for performance reviewer."""
...
class TestPerformancePhaseLoggerIntegration:
    """Test phase logger integration for thinking output."""

    @patch("iron_rook.review.agents.performance.SimpleReviewAgentRunner")
    @pytest.mark.asyncio
    async def test_intake_phase_returns_valid_output(self, mock_runner_class):
        ...
        mock_runner.run_with_retry.return_value = """{
            "phase": "intake",
            "data": {
                "summary": "Performance changes detected",
...
class TestPerformanceFilePatternsAndTools:
    """Test performance reviewer file patterns and tools."""

    def test_get_relevant_file_patterns_returns_performance_patterns(self):
        """Verify get_relevant_file_patterns returns performance-related patterns."""
        from iron_rook.review.agents.performance import PerformanceReliabilityReviewer

        reviewer = PerformanceReliabilityReviewer()
        patterns = reviewer.get_relevant_file_patterns()

        # Verify performance-relevant patterns
        assert "**/*.py" in patterns

    def test_get_allowed_tools_returns_performance_tools(self):
        """Verify get_allowed_tools returns performance-related tools."""
        from iron_rook.review.agents.performance import PerformanceReliabilityReviewer
...
class TestPerformanceReviewOutputGeneration:
    """Test ReviewOutput generation from performance FSM."""
                    ...
                    "medium": [
                        {
                            ...
                            "description": "Database query in loop may cause performance issues",
                            "evidence": [{"type": "file_ref", "path": "src/test.py"}],
                            "recommendations": ["Use batched query or join"],
                        }
                    ],
                    "low": [],
                },
...
class TestPerformanceInvalidTransitions:
...
class TestPerformanceFullFSMExecution:
