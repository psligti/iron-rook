...
class BaseReviewerAgent(ABC):
    ...
    def __init__(
        ...
        # FSM is now optional - subclasses that need state management
        # should use their own adapter (e.g., WorkflowFSMAdapter)
        self._fsm: object | None = None
    ...
    def state(self) -> AgentState:
        ...
        Returns:
            The current AgentState. Returns IDLE by default when no FSM is configured.
            Subclasses with their own FSM can override this property.
        """
        # Default to IDLE when no FSM is configured
        # Subclasses that manage their own state should override this
    ...
    def get_valid_transitions(self) -> dict[AgentState, set[AgentState]]:
        ...
        otherwise returns an empty transitions dict (no transitions allowed).
        ...
        # Default: no transitions when no FSM is configured
        return {}

    def _transition_to(self, new_state: AgentState) -> None:
        """Transition the agent to a new state.

        This is now a no-op by default. Subclasses that manage their own FSM
        should override this method to perform actual state transitions.

        Args:
            new_state: The target AgentState to transition to.

        Note:
            Logs a warning when called on the base class without an FSM.
        """
        import logging

        logger = logging.getLogger(__name__)
        logger.debug(
            f"[{self.__class__.__name__}] _transition_to({new_state}) called - "
            "no FSM configured, transition is a no-op"
        )
