"""Release & Changelog Reviewer agent with FSM pattern.
...
class ReleaseChangelogReviewer(BaseReviewerAgent):
    """Release & Changelog reviewer agent with FSM-based review process.

    This agent implements a 5-phase FSM for changelog review:
    - INTAKE: Analyze PR changes for changelog surfaces
    - PLAN: Create TODOs for changelog analysis
    - ACT: Use ChangelogDelegationSkill to dispatch subagents
    ...
    - Version bumps not matching semver conventions
    ...
    async def _run_intake(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Analyzing PR changes for changelog surfaces")
    ...
    async def _run_plan(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "PLAN", "Creating structured changelog TODOs with priorities"
        )
    ...
    async def _run_act(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Delegating changelog todos to subagents")

        from iron_rook.review.skills.changelog_delegation import ChangelogDelegationSkill

        skill = ChangelogDelegationSkill(
    ...
    async def _run_synthesize(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "SYNTHESIZE", "Aggregating and validating changelog findings from ACT"
        )
    ...
    async def _run_check(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "CHECK", "Assessing findings severity and generating final changelog report"
        )
    ...
    def _get_phase_prompt(self, phase: str) -> str:
        return f"""You are the Release & Changelog Review Agent.
...
2. Identify likely changelog surfaces touched.
3. Generate initial changelog risk hypotheses.

Changelog Detection Patterns:
- Changes to user-facing APIs or CLI
- Changes to configuration formats
- Breaking changes to interfaces
- New features or deprecated features
- Version bumps in pyproject.toml, setup.py
- Changes to CHANGELOG.md or similar files
...
""",
            "PLAN": """PLAN Phase:
Task:
1. Create structured changelog TODOs with:
   - Priority (high/medium/low)
   - Scope (paths, files)
   - Category (version_bump, changelog_entry, breaking_changes, migration_guides, release_notes)
   - Acceptance criteria
2. Specify which changelog areas to check.
...
""",
            "ACT": """ACT Phase:
Task:
1. Delegate todos to changelog subagents.
...
4. Synthesize summary of changelog issues found.
...
3. Generate final changelog review report.

Severity Classification Guidelines:
- CRITICAL: Breaking change without migration documentation
- HIGH: User-visible feature without CHANGELOG entry
- MEDIUM: Version bump missing for feature release
- LOW: Minor changelog formatting issues
...
""",
        ...
        if is_early_exit:
            parts.extend(
                [
                    ...
                    "## Early-Exit Note",
                    "",
                    "The ACT phase returned next_phase_request='done', indicating no significant changelog issues.",
    ...
    def _build_review_output_from_check(
        ...
        return ReviewOutput(
            ...
            summary=f"Changelog review complete. {len(findings)} findings. Risk: {overall_risk}",
            severity=output_severity,  # type: ignore[arg-type]
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Analyzed changelog compliance for changed files",
            ),
            checks=[
                Check(
                    name="changelog_analysis",
                    required=True,
                    commands=[],
                    why="Verify changelog compliance and release hygiene",
                )
            ],
            findings=findings,
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Changelog review failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during changelog review FSM",
            ),
            checks=[],
            skips=[
                Skip(
                    name="changelog_review",
                    why_safe=f"Changelog review error: {error_message}",
                    when_to_run="After fixing changelog review error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix changelog review error"],
                should_fix=[],
