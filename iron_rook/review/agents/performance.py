"""Performance & Reliability Reviewer agent with FSM pattern.

Reviews code changes for performance and reliability issues including:
- IO amplification (N+1 queries, excessive API calls)
- Complexity regressions (O(n^2), unbounded loops)
- Retry/backoff/timeout correctness
- Concurrency hazards (race conditions, async misuse)
...
class PerformanceReliabilityReviewer(BaseReviewerAgent):
    """Performance & reliability reviewer agent with FSM-based review process.

    This agent implements a 5-phase FSM for performance review:
    - INTAKE: Analyze PR changes for performance surfaces
    - PLAN: Create TODOs for performance analysis
    - ACT: Use PerformanceDelegationSkill to dispatch subagents
    ...
    - Retry/backoff/timeout correctness
    - Concurrency hazards (race conditions, async misuse)
    - Latency issues and throughput bottlenecks
    """
    ...
    @property
    def state(self) -> str:  # pyright: ignore[reportIncompatibleMethodOverride]
    ...
    async def _run_intake(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Analyzing PR changes for performance surfaces")
    ...
    async def _run_plan(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "PLAN", "Creating structured performance TODOs with priorities"
        )
    ...
    async def _run_act(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Delegating performance todos to subagents")

        from iron_rook.review.skills.performance_delegation import PerformanceDelegationSkill

        skill = PerformanceDelegationSkill(
    ...
    async def _run_synthesize(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "SYNTHESIZE", "Aggregating and validating performance findings from ACT"
        )
    ...
    async def _run_check(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "CHECK", "Assessing findings severity and generating final performance report"
        )
    ...
    def _get_phase_prompt(self, phase: str) -> str:
        return f"""You are the Performance & Reliability Review Agent.

You are in the {phase} phase of the 5-phase performance review FSM.
...
2. Identify likely performance surfaces touched.
3. Generate initial performance risk hypotheses.

Performance Detection Patterns:
- Database queries in loops (N+1 patterns)
- API calls without batching or pagination
- Unbounded loops or recursive functions
...
""",
            "PLAN": """PLAN Phase:
Task:
1. Create structured performance TODOs with:
    - Priority (high/medium/low)
    - Scope (paths, symbols)
    - Category (io_amplification, complexity, concurrency, latency, throughput)
    - Acceptance criteria
2. Specify which performance areas to check.
...
""",
            "ACT": """ACT Phase:
Task:
1. Delegate todos to performance subagents.
2. Each subagent will use tools (grep, read, python profiling) to collect evidence.
...
4. Synthesize summary of performance issues found.
...
3. Generate final performance review report.

Severity Classification Guidelines:
- CRITICAL: Infinite loops, missing retries on critical paths, race conditions with data loss
- HIGH: N+1 queries, missing timeouts, unbounded growth
- MEDIUM: Suboptimal algorithms, missing caching opportunities
...
""",
        ...
        if is_early_exit:
            parts.extend(
                [
                    ...
                    "## Early-Exit Note",
                    "",
                    "The ACT phase returned next_phase_request='done', indicating no significant performance issues.",
    ...
    def _build_review_output_from_check(
        ...
        if overall_risk == "critical" or overall_risk == "high":
            output_severity: Literal["merge", "warning", "critical", "blocking"] = "critical"
        elif overall_risk == "medium" or overall_risk == "low":
            output_severity = "warning"
        else:
            output_severity = "merge"

        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Performance review complete. {len(findings)} findings. Risk: {overall_risk}",
            severity=output_severity,  # pyright: ignore[reportArgumentType]
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Analyzed performance patterns for changed files",
            ),
            checks=[
                Check(
                    ...
                    required=True,
                    commands=[],
                    why="Verify performance patterns and identify bottlenecks",
                )
            ],
            findings=findings,
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Performance review failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during performance review FSM",
            ),
            checks=[],
            skips=[
                Skip(
                    name="performance_review",
                    why_safe=f"Performance review error: {error_message}",
                    when_to_run="After fixing performance review error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix performance review error"],
                should_fix=[],
                notes_for_coding_agent=[f"Performance review failed: {error_message}"],
            ),
            thinking_log=self._thinking_log,
        )
