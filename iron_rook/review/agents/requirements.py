"""Requirements Reviewer agent with FSM pattern.

Reviews code changes for requirements issues including:
...
class RequirementsReviewer(BaseReviewerAgent):
    """Requirements reviewer agent with FSM-based review process.

    This agent implements a 5-phase FSM for requirements review:
    - INTAKE: Analyze PR changes for requirements surfaces
    - PLAN: Create TODOs for requirements analysis
    - ACT: Use RequirementsDelegationSkill to dispatch subagents
    ...
    async def _run_intake(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Analyzing PR changes for requirements surfaces")
    ...
    async def _run_plan(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "PLAN", "Creating structured requirements TODOs with priorities"
        )
    ...
    async def _run_act(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Delegating requirements todos to subagents")

        from iron_rook.review.skills.requirements_delegation import RequirementsDelegationSkill

        skill = RequirementsDelegationSkill(
    ...
    async def _run_synthesize(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "SYNTHESIZE", "Aggregating and validating requirements findings from ACT"
        )
    ...
    async def _run_check(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "CHECK", "Assessing findings severity and generating final requirements report"
        )
    ...
    def _get_phase_prompt(self, phase: str) -> str:
        return f"""You are the Requirements Review Agent.

You are in the {phase} phase of the 5-phase requirements review FSM.

{get_phase_output_schema(phase)}

Your agent name is "requirements".
...
2. Identify likely requirements surfaces touched.
3. Generate initial requirements risk hypotheses.

Requirements Detection Patterns:
- Changes to API contracts or interfaces
...
""",
            "PLAN": """PLAN Phase:
Task:
1. Create structured requirements TODOs with:
   - Priority (high/medium/low)
   - Scope (paths, symbols)
   - Category (acceptance, traceability, completeness, consistency, edge_cases)
   - Acceptance criteria
2. Specify which requirements areas to check.
...
""",
            "ACT": """ACT Phase:
Task:
1. Delegate todos to requirements subagents.
...
4. Synthesize summary of requirements issues found.
...
3. Generate final requirements review report.

Severity Classification Guidelines:
- CRITICAL: Core requirement not met, breaking specification
- HIGH: Missing acceptance criteria, incomplete feature
- MEDIUM: Minor inconsistency, missing documentation
...
""",
        ...
        if is_early_exit:
            parts.extend(
                [
                    ...
                    "## Early-Exit Note",
                    "",
                    "The ACT phase returned next_phase_request='done', indicating no significant requirements issues.",
    ...
    def _build_review_output_from_check(
        ...
        for fsm_severity in ("critical", "high", "medium", "low"):
            ...
            for finding_dict in findings_by_severity.get(fsm_severity, []):
                findings.append(
                    Finding(
                        id=f"req-{len(findings)}-{finding_dict.get('title', 'unknown')[:20]}",
        ...
        return ReviewOutput(
            ...
            summary=f"Requirements review complete. {len(findings)} findings. Risk: {overall_risk}",
            severity=output_severity,  # type: ignore[arg-type]
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Analyzed requirements patterns for changed files",
            ),
            checks=[
                Check(
                    ...
                    required=True,
                    commands=[],
                    why="Verify requirements patterns and acceptance criteria",
                )
            ],
            findings=findings,
            merge_gate=MergeGate(
                ...
                notes_for_coding_agent=[f"Review {len(findings)} requirements findings"],
            ),
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Requirements review failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during requirements review FSM",
            ),
            checks=[],
            skips=[
                Skip(
                    name="requirements_review",
                    why_safe=f"Requirements review error: {error_message}",
                    when_to_run="After fixing requirements review error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix requirements review error"],
                should_fix=[],
                notes_for_coding_agent=[f"Requirements review failed: {error_message}"],
            ),
            thinking_log=self._thinking_log,
        )

    def get_system_prompt(self) -> str:
        return f"""You are the Requirements Review Subagent.
