"""Unit Tests Review Subagent with FSM pattern.

Reviews code changes for test quality and adequacy including:
...
class UnitTestsReviewer(BaseReviewerAgent):
    """Unit tests reviewer agent with FSM-based review process.

    This agent implements a 5-phase FSM for unit test review:
    - INTAKE: Analyze PR changes for test surfaces
    - PLAN: Create TODOs for test analysis
    - ACT: Use UnitTestsDelegationSkill to dispatch subagents
    ...
    - Test quality issues (assertions, mocking, fixtures)
    - Missing edge case coverage
    - Test naming and organization issues
    """
    ...
    async def _run_intake(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Analyzing PR changes for test surfaces")
    ...
    async def _run_plan(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Creating structured test TODOs with priorities")
    ...
    async def _run_act(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Delegating test todos to subagents")

        from iron_rook.review.skills.unit_tests_delegation import UnitTestsDelegationSkill

        skill = UnitTestsDelegationSkill(
    ...
    async def _run_synthesize(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "SYNTHESIZE", "Aggregating and validating test findings from ACT"
        )
    ...
    async def _run_check(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking(
            "CHECK", "Assessing findings severity and generating final test report"
        )
    ...
    def _get_phase_prompt(self, phase: str) -> str:
        return f"""You are the Unit Tests Review Agent.
...
2. Identify likely test surfaces touched.
3. Generate initial test risk hypotheses.
...
- Changes to test/fixture utilities
...
""",
            "PLAN": """PLAN Phase:
Task:
1. Create structured test TODOs with:
   - Priority (high/medium/low)
   - Scope (paths, symbols)
   - Category (coverage, quality, assertions, mocking, fixtures)
   - Acceptance criteria
2. Specify which test areas to check.
...
""",
            "ACT": """ACT Phase:
Task:
1. Delegate todos to test subagents.
2. Each subagent will use tools (grep, read, python for pytest discovery) to collect evidence.
...
4. Synthesize summary of test issues found.
...
3. Generate final test review report.

Severity Classification Guidelines:
- CRITICAL: High-risk change with no tests; broken/flaky tests introduced
- HIGH: Behavior changed with no tests and moderate risk
- MEDIUM: Tests exist but miss an edge case
...
""",
        ...
        if is_early_exit:
            parts.extend(
                [
                    ...
                    "## Early-Exit Note",
                    "",
                    "The ACT phase returned next_phase_request='done', indicating no significant test issues.",
    ...
    def _build_review_output_from_check(
        ...
        return ReviewOutput(
            ...
            summary=f"Unit tests review complete. {len(findings)} findings. Risk: {overall_risk}",
            severity=output_severity,  # type: ignore[arg-type]
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Analyzed test coverage and quality for changed files",
            ),
            checks=[
                Check(
                    name="test_coverage",
                    required=True,
                    commands=[],
                    why="Verify test coverage for changed code",
                )
            ],
            findings=findings,
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Unit tests review failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during unit tests review FSM",
            ),
            checks=[],
            skips=[
                Skip(
                    name="unit_tests_review",
                    why_safe=f"Unit tests review error: {error_message}",
                    when_to_run="After fixing unit tests review error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix unit tests review error"],
                should_fix=[],
