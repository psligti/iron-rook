"""Bridge between AgentRuntime and review agent contracts.

This module provides transformation functions to convert AgentResult
(from AgentRuntime.execute_agent) into ReviewOutput (review contract).
...
def agent_result_to_review_output(
    ...
    try:
        ...
        logger.warning(f"Agent {agent_result.agent_name} returned invalid JSON: {e}")
        return create_fallback_review_output(
            agent_name=agent_result.agent_name,
            context=context,
    ...
    try:
        ...
        logger.warning(f"Agent {agent_result.agent_name} returned invalid ReviewOutput: {e}")
        return create_fallback_review_output(
            agent_name=agent_result.agent_name,
            context=context,
...
class SecurityReviewerAgentRuntimeWrapper:
    ...
    def __init__(
        self,
        agent_runtime: Optional["dawn_kestrel.agents.runtime.AgentRuntime"],
        session_manager: Optional["dawn_kestrel.core.agent_types.SessionManagerLike"],
        verifier: Optional["dawn_kestrel.agents.review.verifier.FindingsVerifier"] = None,
    ):
