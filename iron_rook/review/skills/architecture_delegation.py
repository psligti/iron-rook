"""Architecture Delegation Skill for delegating architecture analysis to subagents.

This skill encapsulates the delegation logic that analyzes architecture todos from
        ...
        "delegate": """Output JSON format:
{
  ...
  "data": {
    "subagent_requests": [
      {
        "todo_id": "1",
        "title": "Check SOLID principles compliance",
        "scope": {"paths": ["src/api/handlers.py"]},
        "arch_category": "solid",
        "tools_to_use": ["grep", "read", "python"],
        "acceptance_criteria": ["Single responsibility verified", "Dependencies properly injected"]
      }
    ],
    "thinking": "Reasoning about architecture delegation decisions"
  },
...
class ArchitectureDelegationSkill(BaseDelegationSkill):
    """Skill for LLM-based delegation of architecture todos to subagents.

    This skill analyzes architecture todos from the plan_todos phase and uses an LLM
    to generate subagent requests. It then executes each subagent and collects results.

    The skill:
    1. Extracts architecture todos from plan_todos phase output
    ...
    def __init__(
        ...
        """Initialize ArchitectureDelegationSkill.
    ...
    def get_agent_name(self) -> str:
        """Return agent name."""
        return "architecture_delegation"

    def get_subagent_class(self) -> Type[BaseReviewerAgent]:
        """Return the subagent class to instantiate for each request.

        Returns:
            The ArchitectureSubagent class (inherits from BaseReviewerAgent)
        """
        from iron_rook.review.subagents.architecture_subagent import ArchitectureSubagent

        return ArchitectureSubagent

    def build_subagent_request(
        self, todo: Dict[str, Any], context: ReviewContext
    ) -> Dict[str, Any]:
        """Build a request dict for an architecture subagent.
        ...
        Returns:
            Request dict that will be passed to ArchitectureSubagent constructor
        """
        return {
            "todo_id": todo.get("id"),
            "title": todo.get("title"),
            "scope": todo.get("scope", {"paths": context.changed_files}),
            "arch_category": todo.get("category", "general"),
    ...
    def get_system_prompt(self) -> str:
        ...
        Returns system prompt instructing LLM on architecture delegation responsibilities.
        """
        return f"""You are Architecture Delegation Skill.

You are in DELEGATE phase of architecture review FSM.

{get_phase_output_schema("delegate")}

Your agent name is "architecture_delegation".

DELEGATE Phase:
Task:
1. For EVERY TODO, produce a subagent request object.
2. Each subagent will use tools (grep, read, python AST analysis) to collect architecture evidence.
3. You MUST populate "subagent_requests" array with one entry per TODO.
4. ALL architecture analysis must be delegated to subagents - populate subagent_requests for every TODO.

Architecture categories to consider:
- solid: SOLID principles compliance (Single Responsibility, Open/Closed, Liskov, Interface Segregation, Dependency Inversion)
- patterns: Design patterns usage and correctness (Factory, Singleton, Strategy, Observer, etc.)
- coupling: Coupling and cohesion analysis
- layers: Layer/tier architecture validation
- dependencies: Dependency direction and injection patterns
- modularity: Module boundaries and interfaces

Output JSON format:
{get_phase_output_schema("delegate")}"""

    async def review(self, context: ReviewContext) -> ReviewOutput:
        """Perform architecture delegation review on given context.
    ...
    def _build_review_output(
        ...
        for result in subagent_results:
            ...
            for finding_dict in subagent_findings:
                ...
                # Map severity for architecture findings
                ...
                findings.append(
                    Finding(
                        ...
                        risk=finding_dict.get(
                            "description",
                            finding_dict.get("risk", "Architecture issue identified"),
                        ),
                        recommendation=finding_dict.get(
                            "recommendation", "Review and address this architecture finding"
                        ),
                    )
                )
        ...
        return ReviewOutput(
            ...
            summary=f"Delegated {len(subagent_results)} architecture subagent tasks, found {len(findings)} findings",
            severity="critical"
            if any(f.severity in ("critical", "blocking") for f in findings)
            else "warning"
            if findings
            ...
            scope=Scope(
                ...
                reasoning="Delegated architecture todos to subagents for detailed analysis",
            ),
            checks=[
                Check(
                    name="architecture_subagent_execution",
                    required=True,
                    commands=[],
                    why="Verify all delegated architecture subagents completed successfully",
                )
            ],
            findings=findings,
            merge_gate=MergeGate(
                ...
                notes_for_coding_agent=[
                    f"Review {len(findings)} architecture findings from {len(subagent_results)} subagents"
                ],
            ),
        )
    ...
    def _build_empty_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary="No architecture todos to delegate",
            severity="merge",
            scope=Scope(
                relevant_files=[],
                reasoning="Empty plan output - no architecture todos found",
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="approve",
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Architecture delegation failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during architecture delegation phase",
            ),
            checks=[],
            skips=[
                Skip(
                    name="architecture_delegation",
                    why_safe=f"Architecture delegation error: {error_message}",
                    when_to_run="After fixing architecture delegation error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix architecture delegation error"],
                should_fix=[],
                notes_for_coding_agent=[
                    f"Architecture delegation failed with error: {error_message}"
                ],
            ),
        )
