"""Performance Delegation Skill for delegating performance analysis to subagents.

This skill encapsulates the delegation logic that analyzes performance todos from
    ...
    "subagent_requests": [
      {
        ...
        "scope": {"paths": ["src/database/queries.py"]},
        "perf_category": "io_amplification",
        "tools_to_use": ["grep", "read", "python"],
        "acceptance_criteria": ["No queries in loops", "Batching used for bulk operations"]
      }
    ],
    "thinking": "Reasoning about performance delegation decisions"
  },
...
class PerformanceDelegationSkill(BaseDelegationSkill):
    """Skill for LLM-based delegation of performance todos to subagents.

    This skill analyzes performance todos from the plan_todos phase and uses an LLM
    to generate subagent requests. It then executes each subagent and collects results.

    The skill:
    1. Extracts performance todos from plan_todos phase output
    ...
    def __init__(
        ...
        """Initialize PerformanceDelegationSkill.
    ...
    def get_subagent_class(self) -> Type[BaseReviewerAgent]:
        ...
        Returns:
            The PerformanceSubagent class (inherits from BaseReviewerAgent)
        """
    ...
    def build_subagent_request(
        ...
        """Build a request dict for a performance subagent.
        ...
        Returns:
            Request dict that will be passed to PerformanceSubagent constructor
        """
        return {
            "todo_id": todo.get("id"),
            "title": todo.get("title"),
            "scope": todo.get("scope", {"paths": context.changed_files}),
            "perf_category": todo.get("category", "general"),
    ...
    def get_system_prompt(self) -> str:
        ...
        Returns system prompt instructing LLM on performance delegation responsibilities.
        """
        return f"""You are Performance Delegation Skill.

You are in DELEGATE phase of performance review FSM.

{get_phase_output_schema("delegate")}

Your agent name is "performance_delegation".

DELEGATE Phase:
Task:
1. For EVERY TODO, produce a subagent request object.
2. Each subagent will use tools (grep, read, python profiling) to collect performance evidence.
3. You MUST populate "subagent_requests" array with one entry per TODO.
4. ALL performance analysis must be delegated to subagents - populate subagent_requests for every TODO.

Performance categories to consider:
- io_amplification: N+1 queries, excessive API calls, missing pagination
- complexity: O(n^2) algorithms, unbounded loops, recursive issues
- concurrency: Race conditions, missing locks, async misuse
- latency: Slow operations, missing caching, timeout handling
- throughput: Bottlenecks, resource contention, backpressure

Output JSON format:
{get_phase_output_schema("delegate")}"""

    async def review(self, context: ReviewContext) -> ReviewOutput:
        """Perform performance delegation review on given context.
    ...
    def _build_review_output(
                ...
                findings.append(
                    Finding(
                        ...
                        risk=finding_dict.get(
                            "description",
                            finding_dict.get("risk", "Performance issue identified"),
                        ),
                        recommendation=finding_dict.get(
                            "recommendation", "Review and address this performance finding"
                        ),
                    )
                )
        ...
        return ReviewOutput(
            ...
            summary=f"Delegated {len(subagent_results)} performance subagent tasks, found {len(findings)} findings",
            severity="critical"
            if any(f.severity in ("critical", "blocking") for f in findings)
            else "warning"
            if findings
            ...
            scope=Scope(
                ...
                reasoning="Delegated performance todos to subagents for detailed analysis",
            ),
            checks=[
                Check(
                    name="performance_subagent_execution",
                    required=True,
                    commands=[],
                    why="Verify all delegated performance subagents completed successfully",
                )
            ],
            findings=findings,
            merge_gate=MergeGate(
                ...
                notes_for_coding_agent=[
                    f"Review {len(findings)} performance findings from {len(subagent_results)} subagents"
                ],
            ),
        )
    ...
    def _build_empty_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary="No performance todos to delegate",
            severity="merge",
            scope=Scope(
                relevant_files=[],
                reasoning="Empty plan output - no performance todos found",
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="approve",
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Performance delegation failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during performance delegation phase",
            ),
            checks=[],
            skips=[
                Skip(
                    name="performance_delegation",
                    why_safe=f"Performance delegation error: {error_message}",
                    when_to_run="After fixing performance delegation error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix performance delegation error"],
                should_fix=[],
                notes_for_coding_agent=[
                    f"Performance delegation failed with error: {error_message}"
                ],
            ),
        )
