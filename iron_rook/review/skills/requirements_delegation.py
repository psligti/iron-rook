"""Requirements Delegation Skill for delegating requirements analysis to subagents.

This skill encapsulates the delegation logic that analyzes requirements todos from
        ...
        "delegate": """Output JSON format:
{
  ...
  "data": {
    "subagent_requests": [
      {
        "todo_id": "1",
        "title": "Check acceptance criteria coverage",
        "scope": {"paths": ["src/api/handlers.py"]},
        "req_category": "acceptance",
        "tools_to_use": ["grep", "read", "file"],
        "acceptance_criteria": ["Feature X implemented", "Error handling covers edge cases"]
      }
    ],
    "thinking": "Reasoning about requirements delegation decisions"
  },
...
class RequirementsDelegationSkill(BaseDelegationSkill):
    """Skill for LLM-based delegation of requirements todos to subagents.

    This skill analyzes requirements todos from the plan_todos phase and uses an LLM
    to generate subagent requests. It then executes each subagent and collects results.

    The skill:
    1. Extracts requirements todos from plan_todos phase output
    ...
    def get_agent_name(self) -> str:
        return "requirements_delegation"

    def get_subagent_class(self) -> Type[BaseReviewerAgent]:
        from iron_rook.review.subagents.requirements_subagent import RequirementsSubagent

        return RequirementsSubagent
    ...
    def build_subagent_request(
        ...
        return {
            ...
            "req_category": todo.get("category", "general"),
    ...
    def get_system_prompt(self) -> str:
        return f"""You are Requirements Delegation Skill.

You are in DELEGATE phase of requirements review FSM.

{get_phase_output_schema("delegate")}

Your agent name is "requirements_delegation".

DELEGATE Phase:
Task:
1. For EVERY TODO, produce a subagent request object.
2. Each subagent will use tools (grep, read, file) to collect requirements evidence.
3. You MUST populate "subagent_requests" array with one entry per TODO.
4. ALL requirements analysis must be delegated to subagents - populate subagent_requests for every TODO.

Requirements categories to consider:
- acceptance: Acceptance criteria coverage and verification
- traceability: Requirements to implementation traceability
- completeness: All requirements implemented
- consistency: Implementation matches specification
- edge_cases: Edge case and error handling coverage
...
{get_phase_output_schema("delegate")}"""
    ...
    def _build_review_output(
                ...
                findings.append(
                    Finding(
                        ...
                        risk=finding_dict.get(
                            "description",
                            finding_dict.get("risk", "Requirements issue identified"),
                        ),
                        recommendation=finding_dict.get(
                            "recommendation", "Review and address this requirements finding"
                        ),
                    )
                )
        ...
        return ReviewOutput(
            ...
            summary=f"Delegated {len(subagent_results)} requirements subagent tasks, found {len(findings)} findings",
            severity="critical"
            if any(f.severity in ("critical", "blocking") for f in findings)
            else "warning"
            if findings
            ...
            scope=Scope(
                ...
                reasoning="Delegated requirements todos to subagents for detailed analysis",
            ),
            checks=[
                Check(
                    name="requirements_subagent_execution",
                    required=True,
                    commands=[],
                    why="Verify all delegated requirements subagents completed successfully",
                )
            ],
            findings=findings,
            merge_gate=MergeGate(
                ...
                notes_for_coding_agent=[
                    f"Review {len(findings)} requirements findings from {len(subagent_results)} subagents"
                ],
            ),
        )
    ...
    def _build_empty_review_output(self, context: ReviewContext) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary="No requirements todos to delegate",
            severity="merge",
            scope=Scope(
                relevant_files=[],
                reasoning="Empty plan output - no requirements todos found",
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="approve",
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Requirements delegation failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during requirements delegation phase",
            ),
            checks=[],
            skips=[
                Skip(
                    name="requirements_delegation",
                    why_safe=f"Requirements delegation error: {error_message}",
                    when_to_run="After fixing requirements delegation error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix requirements delegation error"],
                should_fix=[],
                notes_for_coding_agent=[
                    f"Requirements delegation failed with error: {error_message}"
                ],
            ),
        )
