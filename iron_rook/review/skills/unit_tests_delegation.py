"""Unit Tests Delegation Skill for delegating test analysis to subagents.

This skill encapsulates the delegation logic that analyzes test todos from
        ...
        "delegate": """Output JSON format:
{
  ...
  "data": {
    "subagent_requests": [
      {
        "todo_id": "1",
        "title": "Check test coverage for public APIs",
        "scope": {"paths": ["src/api/handlers.py"]},
        "test_category": "coverage",
        "tools_to_use": ["grep", "read", "python"],
        "acceptance_criteria": ["All public functions have tests", "Tests cover edge cases"]
      }
    ],
    "thinking": "Reasoning about test delegation decisions"
  },
...
class UnitTestsDelegationSkill(BaseDelegationSkill):
    """Skill for LLM-based delegation of test todos to subagents.

    This skill analyzes test todos from the plan_todos phase and uses an LLM
    to generate subagent requests. It then executes each subagent and collects results.

    The skill:
    1. Extracts test todos from plan_todos phase output
    ...
    def __init__(
        ...
        """Initialize UnitTestsDelegationSkill.
    ...
    def get_agent_name(self) -> str:
        """Return agent name."""
        return "unit_tests_delegation"

    def get_subagent_class(self) -> Type[BaseReviewerAgent]:
        """Return the subagent class to instantiate for each request.

        Returns:
            The UnitTestsSubagent class (inherits from BaseReviewerAgent)
        """
        from iron_rook.review.subagents.unit_tests_subagent import UnitTestsSubagent

        return UnitTestsSubagent

    def build_subagent_request(
        self, todo: Dict[str, Any], context: ReviewContext
    ) -> Dict[str, Any]:
        """Build a request dict for a test subagent.
        ...
        Returns:
            Request dict that will be passed to UnitTestsSubagent constructor
        """
        return {
            "todo_id": todo.get("id"),
            "title": todo.get("title"),
            "scope": todo.get("scope", {"paths": context.changed_files}),
            "test_category": todo.get("category", "general"),
    ...
    def get_system_prompt(self) -> str:
        ...
        Returns system prompt instructing LLM on test delegation responsibilities.
        """
        return f"""You are Unit Tests Delegation Skill.

You are in DELEGATE phase of unit tests review FSM.

{get_phase_output_schema("delegate")}

Your agent name is "unit_tests_delegation".

DELEGATE Phase:
Task:
1. For EVERY TODO, produce a subagent request object.
2. Each subagent will use tools (grep, read, python for pytest discovery) to collect test evidence.
3. You MUST populate "subagent_requests" array with one entry per TODO.
4. ALL test analysis must be delegated to subagents - populate subagent_requests for every TODO.

Test categories to consider:
- coverage: Test coverage for changed code
- quality: Test quality (assertions, mocking, fixtures)
- assertions: Assertion completeness and correctness
- mocking: Mock usage and isolation
- fixtures: Fixture design and reuse

Output JSON format:
{get_phase_output_schema("delegate")}"""

    async def review(self, context: ReviewContext) -> ReviewOutput:
        """Perform test delegation review on given context.
    ...
    def _build_review_output(
        ...
        for result in subagent_results:
            ...
            for finding_dict in subagent_findings:
                ...
                findings.append(
                    Finding(
                        id=f"tests-{len(findings)}-{finding_dict.get('title', 'unknown')[:20]}",
                        ...
                        risk=finding_dict.get(
                            "description",
                            finding_dict.get("risk", "Test issue identified"),
                        ),
                        recommendation=finding_dict.get(
                            "recommendation", "Review and address this test finding"
                        ),
                    )
                )
        ...
        return ReviewOutput(
            ...
            summary=f"Delegated {len(subagent_results)} test subagent tasks, found {len(findings)} findings",
            severity="critical"
            if any(f.severity in ("critical", "blocking") for f in findings)
            else "warning"
            if findings
            ...
            scope=Scope(
                ...
                reasoning="Delegated test todos to subagents for detailed analysis",
            ),
            checks=[
                Check(
                    name="test_subagent_execution",
                    required=True,
                    commands=[],
                    why="Verify all delegated test subagents completed successfully",
                )
            ],
            findings=findings,
            merge_gate=MergeGate(
                ...
                notes_for_coding_agent=[
                    f"Review {len(findings)} test findings from {len(subagent_results)} subagents"
                ],
            ),
        )
    ...
    def _build_empty_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary="No test todos to delegate",
            severity="merge",
            scope=Scope(
                relevant_files=[],
                reasoning="Empty plan output - no test todos found",
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="approve",
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Test delegation failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during test delegation phase",
            ),
            checks=[],
            skips=[
                Skip(
                    name="test_delegation",
                    why_safe=f"Test delegation error: {error_message}",
                    when_to_run="After fixing test delegation error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix test delegation error"],
                should_fix=[],
                notes_for_coding_agent=[f"Test delegation failed with error: {error_message}"],
            ),
        )
