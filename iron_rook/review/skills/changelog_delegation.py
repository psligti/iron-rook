"""Changelog Delegation Skill for delegating changelog analysis to subagents.

This skill encapsulates the delegation logic that analyzes changelog todos from
plan phase and generates subagent requests for delegated items.

Domain: changelog compliance, release notes
        ...
        "delegate": """Output JSON format:
{
  ...
  "data": {
    "subagent_requests": [
      {
        "todo_id": "1",
        "title": "Check CHANGELOG.md for new feature entry",
        "scope": {"paths": ["CHANGELOG.md", "pyproject.toml"]},
        "changelog_category": "version_bump",
        "tools_to_use": ["grep", "read", "file"],
        "acceptance_criteria": ["CHANGELOG has entry for feature", "Version bumped appropriately"]
      }
    ],
    "thinking": "Reasoning about changelog delegation decisions"
  },
...
class ChangelogDelegationSkill(BaseDelegationSkill):
    """Skill for LLM-based delegation of changelog todos to subagents.

    This skill analyzes changelog todos from the plan phase and uses an LLM
    to generate subagent requests. It then executes each subagent and collects results.

    The skill:
    1. Extracts changelog todos from plan phase output
    ...
    def get_agent_name(self) -> str:
        return "changelog_delegation"

    def get_subagent_class(self) -> Type[BaseReviewerAgent]:
        from iron_rook.review.subagents.changelog_subagent import ChangelogSubagent

        return ChangelogSubagent
    ...
    def build_subagent_request(
        ...
        return {
            ...
            "changelog_category": todo.get("category", "general"),
    ...
    def get_system_prompt(self) -> str:
        return f"""You are Changelog Delegation Skill.

You are in DELEGATE phase of changelog review FSM.

{get_phase_output_schema("delegate")}

Your agent name is "changelog_delegation".

DELEGATE Phase:
Task:
1. For EVERY TODO, produce a subagent request object.
2. Each subagent will use tools (grep, read, file) to collect changelog evidence.
3. You MUST populate "subagent_requests" array with one entry per TODO.
4. ALL changelog analysis must be delegated to subagents - populate subagent_requests for every TODO.

Changelog categories to consider:
- version_bump: Version number changes (major, minor, patch)
- changelog_entry: CHANGELOG.md updates for user-visible changes
- breaking_changes: Breaking changes documentation
- migration_guides: Migration/upgrade documentation
- release_notes: Release notes updates
...
{get_phase_output_schema("delegate")}"""
    ...
    def _build_review_output(
        ...
        for result in subagent_results:
            ...
            for finding_dict in subagent_findings:
                ...
                findings.append(
                    Finding(
                        id=f"changelog-{len(findings)}-{finding_dict.get('title', 'unknown')[:20]}",
                        ...
                        risk=finding_dict.get(
                            "description",
                            finding_dict.get("risk", "Changelog issue identified"),
                        ),
                        recommendation=finding_dict.get(
                            "recommendation", "Review and address this changelog finding"
                        ),
                    )
                )
        ...
        return ReviewOutput(
            ...
            summary=f"Delegated {len(subagent_results)} changelog subagent tasks, found {len(findings)} findings",
            severity="critical"
            if any(f.severity in ("critical", "blocking") for f in findings)
            else "warning"
            if findings
            ...
            scope=Scope(
                ...
                reasoning="Delegated changelog todos to subagents for detailed analysis",
            ),
            checks=[
                Check(
                    name="changelog_subagent_execution",
                    required=True,
                    commands=[],
                    why="Verify all delegated changelog subagents completed successfully",
                )
            ],
            findings=findings,
            merge_gate=MergeGate(
                ...
                notes_for_coding_agent=[
                    f"Review {len(findings)} changelog findings from {len(subagent_results)} subagents"
                ],
            ),
        )
    ...
    def _build_empty_review_output(self, context: ReviewContext) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary="No changelog todos to delegate",
            severity="merge",
            scope=Scope(
                relevant_files=[],
                reasoning="Empty plan output - no changelog todos found",
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="approve",
    ...
    def _build_error_review_output(
        ...
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Changelog delegation failed: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=context.changed_files,
                reasoning="Error during changelog delegation phase",
            ),
            checks=[],
            skips=[
                Skip(
                    name="changelog_delegation",
                    why_safe=f"Changelog delegation error: {error_message}",
                    when_to_run="After fixing changelog delegation error",
                )
            ],
            findings=[],
            merge_gate=MergeGate(
                decision="block",
                must_fix=["Fix changelog delegation error"],
                should_fix=[],
                notes_for_coding_agent=[f"Changelog delegation failed with error: {error_message}"],
            ),
        )
