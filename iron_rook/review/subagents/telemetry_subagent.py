"""Dynamic telemetry subagent that executes individual telemetry tasks."""
...
class TelemetrySubagent(BaseDynamicSubagent):
    """ReAct-style telemetry subagent that executes a single telemetry task."""
    ...
    def get_agent_name(self) -> str:
        return f"telemetry_subagent_{self._task.get('todo_id', 'unknown')}"
    ...
    def get_domain_prompt(self) -> str:
        return """Focus on finding evidence-based telemetry and observability findings.

Key areas to check:
1. Logging quality (structured logs, proper log levels, correlation IDs)
2. Tracing spans and propagation (if applicable)
3. Metrics coverage (counters, gauges, histograms, cardinality control)
4. Error reporting (meaningful errors, no sensitive data logged)
5. Observability coverage of new workflows and failure modes
6. Performance signals (timing, retries, rate limits, backoff)

Use tools to verify your analysis. Every finding must include:
1. Specific file:line: references
2. Actual code snippets showing telemetry issues
3. Clear recommendation with best practice examples
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Telemetry Subagent.

You execute a single telemetry task assigned by the parent TelemetryMetricsReviewer.
...
{self.get_domain_prompt()}"""
    ...
    def get_relevant_file_patterns(self) -> List[str]:
        ...
        return ["**/*.py", "**/logging/**", "**/metrics/**", "**/tracing/**"]
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing telemetry intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand telemetry task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting telemetry tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing telemetry tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing telemetry results")
    ...
    def _get_default_patterns(self) -> List[str]:
        telemetry_type = self._task.get("telemetry_category", "general")
        patterns = {
            "logging": ["logger", "logging", "log.", "INFO", "ERROR", "WARN", "DEBUG"],
            "metrics": ["metrics", "counter", "gauge", "histogram", "timer"],
            "tracing": ["trace", "span", "propagate", "context"],
            "observability": ["observe", "monitor", "alert", "health"],
        }
        return patterns.get(telemetry_type, ["logger", "logging", "metrics", "trace"])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Telemetry Subagent executing task: {task_title}
...
- "All error paths have logging" - Met: Each except block contains logger call
- "No sensitive data logged" - Met: No passwords/API keys in log statements
...
1. "description": Explain the telemetry issue with specific locations
2. "recommendations": Provide SPECIFIC fixes with best practice examples
...
2. DIMINISHING RETURNS: More iterations rarely help for telemetry tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Telemetry issue {idx}")
            ...
            finding = Finding(
                id=f"tel-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Telemetry task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Telemetry subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Telemetry task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during telemetry analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Telemetry subagent error: {error_message}"],
                should_fix=[],
