"""Dynamic documentation subagent that executes individual documentation tasks."""
...
class DocumentationSubagent(BaseDynamicSubagent):
    """ReAct-style documentation subagent that executes a single documentation task."""
    ...
    def get_agent_name(self) -> str:
        return f"documentation_subagent_{self._task.get('todo_id', 'unknown')}"
    ...
    def get_domain_prompt(self) -> str:
        return """Focus on finding evidence-based documentation findings.

Key areas to check:
1. Docstrings for public functions/classes (missing, incomplete, outdated)
2. Module-level documentation explaining purpose and contracts
3. README updates when behavior changes
4. Configuration documentation (env vars, settings, CLI flags)
5. Examples and edge case documentation

Use tools to verify your analysis. Every finding must include:
1. Specific file:line: references
2. Actual code snippets showing documentation gaps
3. Clear recommendation with documentation examples
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Documentation Subagent.

You execute a single documentation task assigned by the parent DocumentationReviewer.
...
- python: run Python commands (including docstring extraction with ast module)
...
{self.get_domain_prompt()}"""
    ...
    def get_relevant_file_patterns(self) -> List[str]:
        ...
        return ["**/*.py", "README*", "docs/**", "*.md", "pyproject.toml", ".env.example"]
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing documentation intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand documentation task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting documentation tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing documentation tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing documentation results")
        ...
        for tool in tools_to_use:
            try:
                if tool == "grep":
                    ...
                    results["python"] = await self._execute_python_docstring_check(
                        context, repo_root
                    )
                else:
                    results[tool] = {
    ...
    async def _execute_grep(self, patterns: List[str], repo_root: str) -> Dict[str, Any]:
        ...
        for pattern in patterns_to_search[:5]:
            try:
                cmd = ["rg", "-n", "--max-count=50", "-g", "*.py", "-g", "*.md", pattern, repo_root]
    ...
    async def _execute_python_docstring_check(
            ...
            try:
                ...
                if os.path.exists(full_path):
                    ...
                    try:
                        ...
                        docstring_info = self._extract_docstring_info(tree, file_path)
                        results[file_path] = docstring_info
    ...
    def _extract_docstring_info(self, tree: ast.Module, file_path: str) -> Dict[str, Any]:
        info: Dict[str, Any] = {
            "file": file_path,
    ...
    def _get_default_patterns(self) -> List[str]:
        doc_type = self._task.get("documentation_type", "general")
        patterns = {
            "docstrings": ['"""', "'''", "def ", "class "],
            "readme": ["README", "Usage", "Installation", "Getting Started"],
            "config": ["env:", "config", "settings", "OPTIONS"],
            "examples": ["Example", "Usage:", "example", "```"],
        }
        return patterns.get(doc_type, ['"""', "README", "Usage", "Example"])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Documentation Subagent executing task: {task_title}
...
- "All public functions have docstrings" - Met: ast analysis shows docstrings for all public def statements
- "README includes usage section" - Met: README contains section mentioning the command
...
Available tools: grep, read, file, python (ast for docstring extraction)
...
1. "description": Explain the documentation issue with specific locations
2. "recommendations": Provide SPECIFIC fixes with documentation examples
3. "severity": "warning" for minor issues, "critical" for behavior/documentation mismatch
...
2. DIMINISHING RETURNS: More iterations rarely help for documentation tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Documentation issue {idx}")
            ...
            finding = Finding(
                id=f"doc-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Documentation task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Documentation subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Documentation task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during documentation analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Documentation subagent error: {error_message}"],
                should_fix=[],
