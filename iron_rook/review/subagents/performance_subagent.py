"""Dynamic performance subagent that executes individual performance tasks."""
...
class PerformanceSubagent(BaseDynamicSubagent):
    """ReAct-style performance subagent that executes a single performance task."""
    ...
    def get_agent_name(self) -> str:
        return f"performance_subagent_{self._task.get('todo_id', 'unknown')}"
...
Key areas to check:
1. IO amplification (N+1 queries, excessive API calls in loops)
2. Complexity regressions (O(n^2), unbounded loops)
3. Retry/backoff/timeout correctness
4. Concurrency hazards (async misuse, shared mutable state)
5. Memory/CPU hot paths, caching correctness
6. Latency issues and throughput bottlenecks

Use tools to verify your analysis. Every finding must include:
1. Specific file:line: references
2. Actual code snippets showing performance issues
3. Clear recommendation with optimization examples
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Performance Subagent.

You execute a single performance task assigned by the parent PerformanceReliabilityReviewer.
...
- python: run Python commands for profiling analysis
...
{self.get_domain_prompt()}"""
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing performance intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand performance task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting performance tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing performance tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing performance results")
        ...
        for tool in tools_to_use:
            try:
                if tool == "grep":
                    ...
                    results["python"] = await self._execute_python_analysis(context, repo_root)
                else:
                    results[tool] = {
                        "status": "skipped",
        ...
        for file_path in py_files:
            try:
                ...
                if os.path.exists(full_path):
                    with open(full_path, "r") as f:
                        content = f.read()
                    perf_issues = self._analyze_performance_patterns(content, file_path)
                    results[file_path] = perf_issues
                else:
                    results[file_path] = "(file not found)"
            except Exception as e:
                results[file_path] = f"(error: {e})"
    ...
    def _analyze_performance_patterns(self, content: str, file_path: str) -> Dict[str, Any]:
        ...
        for line_num, line in enumerate(lines, 1):
            for pattern, pattern_name in loop_patterns:
                if re.search(pattern, line):
                    indent = len(line) - len(line.lstrip())
                    for next_line in lines[line_num : line_num + 10]:
                        next_indent = len(next_line) - len(next_line.lstrip())
                        if next_indent > indent:
                            for db_pattern, db_name in db_patterns:
                                if re.search(db_pattern, next_line):
                                    issues.append(
                                        {
                                            "type": "potential_n_plus_1",
                                            "line": line_num,
                                            "pattern": f"{pattern_name} with {db_name}",
                                            "snippet": line.strip()[:100],
                                        }
                                    )
                                    break

        unbounded_loops = re.findall(r"while\s+True:", content)
        if unbounded_loops:
            issues.append(
                {
                    "type": "unbounded_loop",
                    "count": len(unbounded_loops),
                    "pattern": "while True",
                }
            )
    ...
    def _get_default_patterns(self) -> List[str]:
        perf_type = self._task.get("perf_category", "general")
        patterns = {
            "io_amplification": [
                "for ",
                "while ",
            ...
            "complexity": ["for ", "while ", "range(", "sorted(", "filter("],
            "concurrency": [
                "async ",
                "await ",
                "Lock(",
            ...
            "latency": ["time.sleep", "await asyncio.sleep", "timeout", "retry", "backoff"],
            "throughput": ["queue", "batch", "chunk", "buffer", "cache"],
        }
        return patterns.get(perf_type, ["for ", "while ", "async ", "await ", ".query("])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Performance Subagent executing task: {task_title}
...
- "No N+1 queries detected" - Met: No database queries inside loops
- "Proper retry logic used" - Met: Exponential backoff with max retries configured
...
Available tools: grep, read, file, python (pattern analysis for N+1, complexity, concurrency)
...
1. "description": Explain the performance issue with specific locations
2. "recommendations": Provide SPECIFIC fixes with optimization examples
...
2. DIMINISHING RETURNS: More iterations rarely help for performance tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Performance issue {idx}")
            ...
            finding = Finding(
                id=f"perf-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Performance task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Performance subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Performance task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during performance analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Performance subagent error: {error_message}"],
                should_fix=[],
