"""Dynamic changelog subagent that executes individual changelog tasks.
...
class ChangelogSubagent(BaseDynamicSubagent):
    """ReAct-style changelog subagent that executes a single changelog task."""
    ...
    def get_agent_name(self) -> str:
        return f"changelog_subagent_{self._task.get('todo_id', 'unknown')}"
    ...
    def get_domain_prompt(self) -> str:
        return """Focus on finding evidence-based changelog findings.

Key areas to check:
1. CHANGELOG.md presence and updates for user-visible changes
2. Version bumps (major, minor, patch) in pyproject.toml, setup.py, etc.
3. Breaking changes documentation
4. Migration guides for breaking changes
5. Release notes accuracy

Use tools to verify your analysis. Every finding must include:
1. Specific file:line: references
2. Actual content showing changelog issues
3. Clear recommendation with examples
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Changelog Subagent.

You execute a single changelog task assigned by the parent ReleaseChangelogReviewer.
...
{self.get_domain_prompt()}"""
    ...
    def get_relevant_file_patterns(self) -> List[str]:
        ...
        return ["CHANGELOG*", "pyproject.toml", "setup.py", "setup.cfg", "**/__init__.py"]
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing changelog intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand changelog task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting changelog tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing changelog tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing changelog results")
    ...
    def _get_default_patterns(self) -> List[str]:
        changelog_type = self._task.get("changelog_category", "general")
        patterns = {
            "version_bump": ["version", "VERSION", "__version__", "0\\.", "1\\."],
            "changelog_entry": ["##", "###", "Added", "Changed", "Fixed", "Deprecated"],
            "breaking_changes": ["BREAKING", "breaking", "migration", "upgrade"],
            "migration_guides": ["migration", "upgrade", "MIGRATION"],
            "release_notes": ["Release", "release", "RELEASE", "v[0-9]"],
        }
        return patterns.get(changelog_type, ["version", "CHANGELOG", "release"])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Changelog Subagent executing task: {task_title}
...
- "CHANGELOG.md has entry for new feature" - Met: CHANGELOG.md contains feature description
- "Version bumped appropriately" - Met: pyproject.toml version increased
...
1. "description": Explain the changelog issue with specific locations
...
2. DIMINISHING RETURNS: More iterations rarely help for changelog tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Changelog issue {idx}")
            ...
            finding = Finding(
                id=f"changelog-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Changelog task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Changelog subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Changelog task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during changelog analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Changelog subagent error: {error_message}"],
                should_fix=[],
