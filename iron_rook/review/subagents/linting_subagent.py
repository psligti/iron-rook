"""Dynamic linting subagent that executes individual linting tasks."""
...
class LintingSubagent(BaseDynamicSubagent):
    """ReAct-style linting subagent that executes a single linting task."""
    ...
    def get_agent_name(self) -> str:
        return f"linting_subagent_{self._task.get('todo_id', 'unknown')}"
    ...
    def get_domain_prompt(self) -> str:
        return """Focus on finding evidence-based linting findings.

Key areas to check:
1. Code style issues (formatting, indentation, line length)
2. Type hint coverage and quality
...
2. Actual code snippets showing lint issues
3. Clear recommendation with examples
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Linting Subagent.

You execute a single linting task assigned by the parent LintingReviewer.
...
- python: run Python commands (including lint tools like ruff, black, mypy)
...
{self.get_domain_prompt()}"""
    ...
    def get_relevant_file_patterns(self) -> List[str]:
        ...
        return ["**/*.py", "*.toml", "*.yaml", "*.yml", "pyproject.toml", "ruff.toml", ".flake8"]
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing linting intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand linting task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting linting tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing linting tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing linting results")
        ...
        for tool in tools_to_use:
            try:
                if tool == "grep":
                    ...
                    results["python"] = await self._execute_python_lint_check(context, repo_root)
                else:
                    results[tool] = {
                        "status": "skipped",
    ...
    async def _execute_python_lint_check(
            ...
            try:
                ...
                if os.path.exists(full_path):
                    ...
                    try:
                        ...
                        lint_info = self._extract_lint_info(tree, file_path)
                        results[file_path] = lint_info
    ...
    def _extract_lint_info(self, tree: ast.Module, file_path: str) -> Dict[str, Any]:
        info: Dict[str, Any] = {
            "file": file_path,
            "functions": [],
            "classes": [],
    ...
    def _get_default_patterns(self) -> List[str]:
        lint_type = self._task.get("lint_type", "general")
        patterns = {
            "type_hints": ["def ", ": ", "-> "],
            "imports": ["import ", "from "],
            "formatting": ["    ", "  "],
        ...
        return patterns.get(lint_type, ["def ", "class ", "import "])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Linting Subagent executing task: {task_title}
...
- "All public functions have type hints" - Met: AST analysis shows type annotations for all public def statements
- "No unused imports" - Met: grep analysis shows all imports are referenced
...
Available tools: grep, read, file, python (ast for code analysis, ruff/black for linting)
...
1. "description": Explain the lint issue with specific locations
2. "recommendations": Provide SPECIFIC fixes with code examples
3. "severity": "warning" for minor issues, "critical" for serious violations
...
2. DIMINISHING RETURNS: More iterations rarely help for linting tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Linting issue {idx}")
            ...
            finding = Finding(
                id=f"lint-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Linting task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Linting subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Linting task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during linting analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Linting subagent error: {error_message}"],
                should_fix=[],
