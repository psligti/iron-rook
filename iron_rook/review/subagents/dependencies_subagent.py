"""Dynamic dependencies subagent that executes individual dependency tasks."""
...
class DependenciesSubagent(BaseDynamicSubagent):
    """ReAct-style dependencies subagent that executes a single dependency task."""
    ...
    def get_agent_name(self) -> str:
        return f"dependencies_subagent_{self._task.get('todo_id', 'unknown')}"
    ...
    def get_domain_prompt(self) -> str:
        return """Focus on finding evidence-based dependency findings.

Key areas to check:
1. License compatibility (MIT, Apache, GPL, etc.)
2. Security advisories and known vulnerabilities
3. Version pinning and reproducibility
4. Supply chain risks (typosquatting, untrusted packages)
5. Build reproducibility (lockfile consistency)

Use tools to verify your analysis. Every finding must include:
1. Specific file:line: references
2. Actual package names and versions
3. Clear recommendation with specific actions
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Dependencies Subagent.

You execute a single dependency task assigned by the parent DependencyLicenseReviewer.
...
{self.get_domain_prompt()}"""
    ...
    def get_relevant_file_patterns(self) -> List[str]:
        ...
        return ["pyproject.toml", "requirements*.txt", "poetry.lock", "uv.lock"]
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing dependency intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand dependency task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting dependency tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing dependency tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing dependency results")
    ...
    def _get_default_patterns(self) -> List[str]:
        dep_type = self._task.get("dep_category", "general")
        patterns = {
            "license": ["license", "MIT", "Apache", "GPL", "copyright"],
            "security": ["vulnerability", "CVE", "advisory", "security"],
            "versions": ["version", "pin", "require", "dependency"],
            "supply_chain": ["source", "repository", "hash", "checksum"],
            "audit": ["lock", "hash", "frozen", "pin"],
        }
        return patterns.get(dep_type, ["version", "require", "dependency"])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Dependencies Subagent executing task: {task_title}
...
- "All licenses compatible with MIT" - Met: No GPL or copyleft licenses found
- "No known vulnerabilities" - Met: pip-audit returns 0 issues
...
1. "description": Explain the dependency issue with specific locations
...
2. DIMINISHING RETURNS: More iterations rarely help for dependency tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Dependency issue {idx}")
            ...
            finding = Finding(
                id=f"dep-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Dependency task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Dependency subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Dependency task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during dependency analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Dependency subagent error: {error_message}"],
                should_fix=[],
