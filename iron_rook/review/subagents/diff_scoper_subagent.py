"""Dynamic diff scoper subagent that executes individual diff scope analysis tasks."""
...
class DiffScoperSubagent(BaseDynamicSubagent):
    """ReAct-style diff scoper subagent that executes a single diff scope analysis task.
    ...
    def get_agent_name(self) -> str:
        return f"diff_scoper_subagent_{self._task.get('todo_id', 'unknown')}"
    ...
    def get_domain_prompt(self) -> str:
        return """Focus on finding evidence-based diff scope findings.

Key areas to analyze:
1. Change magnitude and complexity (lines added/removed, files touched)
2. Cross-module dependencies and impact areas
3. Risk level assessment (high/medium/low)
4. Routing recommendations for other reviewers
5. Scope boundaries and affected subsystems

Use tools to verify your analysis. Every finding must include:
1. Specific file:line: references
2. Actual code snippets showing impact
3. Clear recommendation with risk assessment
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Diff Scoper Subagent.

You execute a single diff scope analysis task assigned by the parent DiffScoperReviewer.
...
- python: run Python commands (including AST analysis)
...
{self.get_domain_prompt()}"""
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing diff scope intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand diff scope task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting diff scope tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing diff scope tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing diff scope results")
    ...
    def _get_default_patterns(self) -> List[str]:
        scope_type = self._task.get("scope_category", "general")
        patterns = {
            "impact": ["import ", "from ", "class ", "def "],
            "risk": ["TODO", "FIXME", "XXX", "HACK"],
            "routing": ["test", "spec", "config"],
            "scope": ["import ", "from ", "class ", "module"],
        }
        return patterns.get(scope_type, ["import ", "from ", "class ", "def "])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Diff Scoper Subagent executing task: {task_title}
...
- "Change scope identified" - Met: All affected modules and files documented
- "Risk level assessed" - Met: Risk classification with rationale provided
...
Available tools: grep, read, file, python (AST for structure/dependency analysis)
...
1. "description": Explain the diff scope issue with specific locations
2. "recommendations": Provide SPECIFIC fixes with examples
...
2. DIMINISHING RETURNS: More iterations rarely help for diff scope tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Diff scope issue {idx}")
            ...
            finding = Finding(
                id=f"diffscope-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Diff scope task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Diff scope subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Diff scope task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during diff scope analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Diff scope subagent error: {error_message}"],
                should_fix=[],
