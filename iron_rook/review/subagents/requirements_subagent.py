"""Dynamic requirements subagent that executes individual requirements tasks.
...
class RequirementsSubagent(BaseDynamicSubagent):
    """ReAct-style requirements subagent that executes a single requirements task."""
    ...
    def get_agent_name(self) -> str:
        return f"requirements_subagent_{self._task.get('todo_id', 'unknown')}"

    def get_domain_tools(self) -> List[str]:
        return ["grep", "read", "file"]

    def get_allowed_tools(self) -> List[str]:
        return self.get_domain_tools()

    def get_domain_prompt(self) -> str:
        return """Focus on finding evidence-based requirements findings.

Key areas to check:
1. Acceptance criteria coverage - verify each criterion is implemented
2. Requirements traceability - link code to specification
3. Implementation completeness - all features present
4. Consistency - implementation matches specification
5. Edge cases - error handling and boundary conditions covered

Use tools to verify your analysis. Every finding must include:
1. Specific file:line: references
2. Actual code snippets showing requirements issues
3. Clear recommendation with specific fixes
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Requirements Subagent.

You execute a single requirements task assigned by the parent RequirementsReviewer.
...
{self.get_domain_prompt()}"""
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing requirements intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand requirements task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting requirements tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing requirements tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing requirements results")
    ...
    def _get_default_patterns(self) -> List[str]:
        req_type = self._task.get("requirements_type", "general")
        patterns = {
            "acceptance": ["TODO", "FIXME", "acceptance", "criterion", "requirement"],
            "traceability": ["requirement", "spec", "implements", "feature"],
            "completeness": ["TODO", "FIXME", "unimplemented", "stub"],
            "edge_cases": ["error", "exception", "edge", "boundary", "validate"],
        }
        return patterns.get(req_type, ["TODO", "FIXME", "requirement", "feature"])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Requirements Subagent executing task: {task_title}
...
- "Feature X implemented" - Met: Function X exists and is called
- "Error handling covers edge cases" - Met: All error paths have handlers
...
Available tools: grep, read, file

PLAN REQUIREMENTS:
1. "tools_to_use": List 2-3 specific tools
...
1. "description": Explain the requirements issue with specific locations
2. "recommendations": Provide SPECIFIC fixes
3. "severity": "warning" for minor issues, "critical" for missing requirements
...
2. DIMINISHING RETURNS: More iterations rarely help for requirements tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Requirements issue {idx}")
            ...
            finding = Finding(
                id=f"req-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Requirements task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Requirements subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Requirements task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during requirements analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Requirements subagent error: {error_message}"],
                should_fix=[],
