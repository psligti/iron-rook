"""Dynamic unit tests subagent that executes individual test analysis tasks."""
...
class UnitTestsSubagent(BaseDynamicSubagent):
    """ReAct-style unit tests subagent that executes a single test task."""
    ...
    def get_agent_name(self) -> str:
        return f"unit_tests_subagent_{self._task.get('todo_id', 'unknown')}"
    ...
    def get_domain_prompt(self) -> str:
        return """Focus on finding evidence-based test findings.

Key areas to check:
1. Test coverage for changed code (missing tests, uncovered paths)
2. Test quality (assertions, edge cases, determinism)
3. Test fixtures and mocking (proper isolation, fixture reuse)
4. Test naming and organization (clear test names, proper file structure)

Use tools to verify your analysis. Every finding must include:
1. Specific file:line: references
2. Actual code snippets showing test gaps or issues
3. Clear recommendation with test examples
"""

    def get_system_prompt(self) -> str:
        task_json = json.dumps(self._task, indent=2)
        return f"""You are a Unit Tests Subagent.

You execute a single test task assigned by the parent UnitTestsReviewer.
...
- python: run Python commands (including pytest discovery)
...
{self.get_domain_prompt()}"""
    ...
    def get_relevant_file_patterns(self) -> List[str]:
        ...
        return ["**/*.py", "tests/**", "test_*.py", "*_test.py"]
    ...
    async def _run_intake_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("INTAKE", "Capturing test intent")
        ...
        frame = ThinkingFrame(
            state="intake",
            goals=["Understand test task", "Capture acceptance criteria"],
    ...
    async def _run_plan_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("PLAN", "Selecting test tools")
    ...
    async def _run_act_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("ACT", "Executing test tools")
    ...
    async def _run_synthesize_phase(self, context: ReviewContext) -> Dict[str, Any]:
        self._phase_logger.log_thinking("SYNTHESIZE", "Analyzing test results")
        ...
        for tool in tools_to_use:
            try:
                if tool == "grep":
                    ...
                    results["python"] = await self._execute_pytest_discovery(context, repo_root)
                else:
                    results[tool] = {
                        "status": "skipped",
    ...
    async def _execute_pytest_discovery(
            ...
            try:
                ...
                if os.path.exists(full_path):
                    ...
                    try:
                        ...
                        test_info = self._extract_test_info(tree, file_path)
                        results[file_path] = test_info
    ...
    def _extract_test_info(self, tree: ast.Module, file_path: str) -> Dict[str, Any]:
        info: Dict[str, Any] = {
            "file": file_path,
            "test_functions": [],
            "test_classes": [],
    ...
    def _get_default_patterns(self) -> List[str]:
        test_type = self._task.get("test_type", "general")
        patterns = {
            "coverage": ["def ", "class ", "test_", "assert "],
            "quality": ["assert ", "mock", "fixture", "patch"],
            "assertions": ["assert ", "assertEqual", "assertTrue", "pytest.raises"],
            "mocking": ["Mock", "patch", "mock", "MagicMock"],
            "fixtures": ["@pytest.fixture", "fixture", "conftest"],
        }
        return patterns.get(test_type, ["def ", "test_", "assert "])
    ...
    def _build_intake_prompt(self, task_title: str, task_json: str) -> str:
        return f"""You are a Unit Tests Subagent executing task: {task_title}
...
- "All public functions have corresponding tests" - Met: test file exists with test_ functions matching public API
- "Edge cases are covered" - Met: tests include boundary values and error conditions
...
Available tools: grep, read, file, python (pytest discovery)
...
1. "description": Explain the test issue with specific locations
2. "recommendations": Provide SPECIFIC fixes with test examples
3. "severity": "warning" for minor issues, "critical" for missing/broken tests
...
2. DIMINISHING RETURNS: More iterations rarely help for test analysis tasks
...
"""
    ...
    def _build_review_output(self, context: ReviewContext) -> ReviewOutput:
        ...
        for idx, finding_dict in enumerate(self._accumulated_findings):
            title = finding_dict.get("title", f"Test issue {idx}")
            ...
            finding = Finding(
                id=f"test-{self._task.get('todo_id')}-{idx}",
                title=title,
        ...
        summary = synthesize_output.get(
            "summary", f"Test task completed in {self._iteration_count} iterations"
        )
        ...
        return ReviewOutput(
            ...
            scope=Scope(
                ...
                ignored_files=[],
                reasoning=f"Test subagent for task {todo_id} ({self._iteration_count} iterations)",
            ),
            findings=findings,
    ...
    def _build_error_output(self, context: ReviewContext, error_message: str) -> ReviewOutput:
        return ReviewOutput(
            agent=self.get_agent_name(),
            summary=f"Test task failed at iteration {self._iteration_count}: {error_message}",
            severity="critical",
            scope=Scope(
                relevant_files=[], ignored_files=[], reasoning="Error during test analysis"
            ),
            findings=[],
            merge_gate=MergeGate(
                decision="needs_changes",
                must_fix=[f"Test subagent error: {error_message}"],
                should_fix=[],
